...
`app/app.js`
 - Remove all references to `il18n` and `LanguageProvider`
 - Remove function wrapper around `ReactDOM.render(...);`
 - Remove Hot reloadable translation 
 - Remove chunked polyfill

`app/reducers.js`
 - Remove `languageProviderReducer` and its references
