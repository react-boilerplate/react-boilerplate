...
following in their [`app/app.js`](https://github.com/react-boilerplate/react-boilerplate/blob/master/app/app.js) file:
...
However, they can make debugging deployments more challenging so, service workers are opt-in.
...
pre-cache all of your local assets and keep them up to date as you deploy updates.
...
1. After the initial caching is done, the [service worker lifecycle](https://developers.google.com/web/fundamentals/primers/service-workers/lifecycle)
   controls when updated content ends up being shown to users. In order to guard against race conditions with lazy-loaded content,
...
2. Users aren't always familiar with offline-first web apps. It can be useful to
   ...
   starting point, you can make use of the logic included in [`app/serviceWorker.js`](https://github.com/react-boilerplate/react-boilerplate/blob/master/app/serviceWorker.js), which
...
4. The service worker is only enabled in the production environment, e.g. the output of `npm run build`. It's recommended that you do not enable an
...
5. If you _need_ to test your offline-first service worker locally, build
   ...
   build directory. After running the build script, open the browser in the created URL. Be sure to always use an incognito window to avoid complications with your browser cache.

6. By default, the generated service worker file will not intercept or cache any
   cross-origin traffic, like HTTP API requests, images, or embeds loaded from a different domain.
