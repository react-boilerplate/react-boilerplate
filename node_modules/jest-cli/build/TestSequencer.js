...
class TestSequencer {
  ...
  _getCachePath(context) {
    ...
    return (0, (_jestHasteMap || _load_jestHasteMap()).getCacheFilePath)(
      ...
      if ((_fs || _load_fs()).default.existsSync(cachePath)) {
        try {
          this._cache.set(
            context,
            JSON.parse(
              (_fs || _load_fs()).default.readFileSync(cachePath, 'utf8')
            )
          );
        } catch (e) {}
      }
  ...
  sort(tests) {
    ...
    const fileSize = test =>
      stats[test.path] ||
      (stats[test.path] = (_fs || _load_fs()).default.statSync(test.path).size);
